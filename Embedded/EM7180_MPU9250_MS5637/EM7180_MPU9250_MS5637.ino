/* EM7180_MPU9250_MS5637_t3 Basic Example Code
 by: Kris Winer
 ...
 4k7 resistors are on the EM7180+MPU9250+MS5637+M24512DRC Mini Add-On board for Teensy 3.1.
  ...
  // For more see http://en.wikipedia.org/wiki/Conversion_between_quaternions_and_Euler_angles which has additional links.
   ...
   yaw   = atan2(2.0f * (q[1] * q[2] + q[0] * q[3]), q[0] * q[0] + q[1] * q[1] - q[2] * q[2] - q[3] * q[3]);   
    ...
    Serial.println(Roll, 2);
      ...
      if(passThru) Serial.print( (float) Temperature, 2); 
      if(!passThru) Serial.print( temperature, 2); 
      Serial.println(" C"); // temperature in degrees Celsius
      Serial.print("Altimeter temperature = "); 
      if(passThru) Serial.print(9.*((float)Temperature)/5. + 32., 2); 
      if(!passThru) Serial.print(9.*temperature/5. + 32., 2); 
      Serial.println(" F"); // temperature in degrees Fahrenheit
      Serial.print("Altimeter pressure = "); 
      if(passThru) Serial.print((float)Pressure, 2);  
      if(!passThru) Serial.print(pressure, 2);  
      Serial.println(" mbar");// pressure in millibar
      if(passThru) altitude = 145366.45f*(1.0f - pow((((float)Pressure)/1013.25f), 0.190284f));
      if(!passThru) altitude = 145366.45f*(1.0f - pow(((pressure)/1013.25f), 0.190284f));
